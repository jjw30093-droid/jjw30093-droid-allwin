...
- 赔率涨跌方向（2026-08-26 赔率展示重做）用专用 token `--odds-up`/`--odds-down`，
  **红涨绿跌，经站长明确拍板**（复合中国用户习惯；2026-08-26 首版本曾刻意
  避红避橙、改用青绿/板岩蓝，理由是怕跟"红=真实错误"的锁死语义冲突——站长
  随后明确要求改回红涨绿跌，本条即改判后的最终结论，不是待定状态）。
  与全站"红=真实错误或不可用"的红**不是同一个色值**：`--odds-up` 特意选了
  比 `--brand-red` 更深的一档（浅色 `#b3261e`/深色 `#ff8073`），肉眼仍读作
  "红"，但不会跟错误态的红字节级重合；`--odds-down` 同理避开 `--win` 的
  青绿家族，用深绿/浅绿（`#1b5e20`/`#6fcf7a`）。方向仍要求**颜色 + 箭头
  (↑/↓) + 带符号幅度三通道冗余**，色盲用户不依赖颜色也能读——红绿撞色是
  最常见的色盲混淆对，双通道冗余在这个配色下比之前更重要，不能省。两个
  方向色对 surface/surface-soft/surface-2 三种底、深浅两套主题实测均
  ≥4.4:1。徽标类小字号最易漏检对比度（字号最小 + 底色最浅），改配色时的
  合成对比度断言必须覆盖徽标，不只覆盖正文数字。
...
### 11.5 参照外部产品做视觉改造的纪律（2026-09-11，经真实返工确认）

站长提供 FotMob 安卓包，要求"解析联赛 team stats / player stats 的 UI/UX，
在它的基础上优化我们的对应页面"。反编译 + 计划 + 实现 + 924 条测试全绿 +
部署上线，结果站长一眼看出做的和原版完全不是一回事——FotMob 是卡内纵向
top-3 列表，做出来的是横向可滑动卡片轮播。**排版和排序都不一样，整块返工。**

失败链条四环，错在前三环，第四环没兜住：

- **弱证据当强证据。** 从 dex/resources 里搜到 `include_top_players.xml`
  （`RecyclerView` + `NestedScrollableHost`，即横向轮播），凭文件名对得上
  就认定它是联赛 Stats tab 的布局。它其实是另一个 widget。**从未验证过这个
  layout 到底被哪个 fragment/adapter inflate**——靠文件名猜，然后当成实证
  写进了计划。
- **推翻了项目自己的设计真源。** `DESIGN.md:195` 早已写明 FotMob 的准确
  形态（"卡内展示 top-3……每行：排名 + 队徽或头像 + 姓名/队名 + 数值胶囊……
  卡底部一行「全部 →」"），这是之前审过的结论。计划里却**专门写了一段论证
  为什么不做纵向 top-3、改做横滑**，理由是首页横滑的先例（首页那块只有
  5-7 张卡、信息密度完全不同，不可类比）。等于用一条误认的 XML + 一条不
  相干的先例，推翻了一条强证据。这一步是根因。
- **计划里没有"对着原版核对"这一环。** 计划写满了实现细节（卡宽 104px、
  `scroll-snap-type`、断点策略），唯独没有一句"成品长什么样 vs 原版长什么
  样"。更糟的是计划里还明写了"断点策略是与 FotMob 参考的刻意偏离点，
  commit message 里要说明这条偏离"——**当时已经意识到在偏离原版，却把它
  当成可以自己拍板的实现细节**，没回来问站长。
- **验收全绿，但没有一条在测"长得像不像"。** tsc / eslint / 924 条 vitest /
  build 全过，这些只能证明"没崩""元素存在""类型对"，证明不了排版对。计划里
  写了"起本地预览截图确认"，实际跳过了——**没打开过页面就报了完工**。

据此固化：

- **`DESIGN.md` 已经写死的形态不得自行重新解释。** 要偏离必须先问站长，不
  允许在计划里自己论证、自己批准。计划文档不是授权来源，站长的回复才是。
- **从 APK / 反编译产物取的 layout，必须确认它真的被目标页面使用**（追到
  inflate 它的 fragment/adapter/screen），文件名或资源名对得上不算证据。
  做不到这一步就如实标 `UNVERIFIED`，不得当成实证写进计划。
- **凡是"参照某个外部产品做视觉改造"的任务，验收第一条是打开页面与原版
  并排比对**，不是测试绿。测试绿是必要条件，不是完成条件——jsdom 不跑 CSS，
  单测结构上就抓不到排版类问题（同一轮返工里还踩到一次：`.list` 的
  `display:flex` 是作者样式、优先级高于 UA 隐藏 `<details>` 子节点的规则，
  折叠态下那段 `ol` 仍占布局，真机实测卡片自身 192px、被折叠的 `ol` 仍报
  298px，只能靠真实浏览器发现，见
  `frontend/components/LeaderboardCard.module.css` 的 `.foldedList` 注释）。
- **没有实际打开过页面，不得报告视觉类任务完成**（§2.2 真实输出纪律在视觉
  改造场景下的具体化）。
