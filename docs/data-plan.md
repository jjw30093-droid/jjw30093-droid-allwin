...
> 2026-08-25:下文提到的 WDL 模型 / `gold_wdl_predictions` / `predict_wdl_future.py` /
> 正式预测登记簿已整体废弃并删除代码（胜率改由 bet365 赔率直接派生）；相关段落
> 均为历史记录，不代表当前状态或排期。
...
> **2026-08-20 次级联赛回填(经用户批准执行)**:上一条 2026-08-10 记录称
> 48/57/61/268 在 `dim_match` 中 0 数据——**已过时**,当时其实已经异步落库了
> 26/27 赛程骨架(此前哪一轮做的未见记录,以生产库实测为准)。本轮新做的是:
>
> - **26/27 升级到五大联赛的 14 支球队**(英超 3/法甲 2/德甲 3/意甲 3/西甲 3,
>   均已生产库核实、25/26 赛季在系统里 0 场完赛)按各自最后 10 轮回填历史:
>   英冠 48(已注册)120 场、**德乙 146**(2. Bundesliga)90 场、
>   **西乙 140**(LaLiga2)110 场、**意乙 86**(Serie B)100 场、
>   **法乙 110**(Ligue 2)90 场。德乙/西乙/意乙/法乙的 FotMob league id
>   之前全仓库未出现过,本轮真实网络探测确证(`details.name`/`country` 核对
>   一致),但**刻意不写入 `LEAGUE_META`**——只做历史数据源,不注册为可浏览
>   联赛(验证 `/api/v1/matches` 全量分页无泄漏、直接访问详情页 404)。
> - **57 荷甲 / 61 葡超**(已注册,此前只有 26/27 赛程骨架、0 完赛)补齐
>   25/26 完整赛季:荷甲 309 场完赛 + 积分榜,葡超 306 场完赛 + 积分榜。
>
> 采集全程 0 失败(1,126 场)。**关键架构限制**(不是本轮修复范围,记录以防
> 误判"回填了就该起作用"):`backend/queries/window.py` 等历史查询按
> **当前比赛所属联赛** `League_ID` 过滤,不按球队跨联赛取历史——所以升级
> 球队在新赛季比赛页的"数据"tab(进攻转化链/控球/防守承压/风格四象限)
> 依然是空的,回填只让 (a) 射门图(`matches.py::recent_shot_map_spec` 本身
> league-agnostic,前端默认口径本轮改为按球队有数据的口径自动选择,不再
> 死守"同联赛")、(b) 荷甲/葡超**自己的**联赛页(赛程/积分榜/球队数据)
> 生效。`backend/silver/build_silver.py` 新增 `PARTIAL_SEASON_PARTITIONS`
> 跳过名单(当前仅 `(48,"2025/2026")`),防止"最后 10 轮"的部分赛季数据被
> 聚合成看起来像完整赛季的场均值/进球时间分桶。
>
> 42/73/10216 欧战三项仍为 0 数据(季外,见下方历史记录),未变。

> **2026-08-10 数据管道重建 · Phase 1 登记**:`LEAGUE_META` 由 10 → **17 联赛**,
> 新增 48 英冠 / 57 荷甲 / 61 葡超 / 268 巴甲 / 42 欧冠 / 73 欧联 / 10216 欧协联,
> 全部 `league:lottery`(匿名可浏览)。Phase 0 真实网络已确证 `details.id` 与赛季
> (见 `docs/data-sources.md` §1.2)。**这 7 项当前在 `dim_match` 中 0 数据**——
> 向前采集在 Phase 2(T+7 赛程 job)落库,历史回填本轮不做。因此:
> ① 它们的 sitemap/`ANON_LEAGUES` 收录**延后到数据真正落库后**(4 个在季的域内联赛
> 随 Phase 2,欧战三项待其 9 月开季),避免爬虫抓到空壳(§8.3);
> ② 下方覆盖矩阵尚未包含这 7 项,待 Phase 2 采集后按 `data_coverage.py` 重跑补入。
>
> ⚠️ 上一条记录的"0 数据"断言已被 2026-08-20 记录更正,下方覆盖矩阵本身
> (as-of 2026-08-05)仍未包含 48/57/61/268/146/140/86/110,如需当前真实
> 场次数请重跑 `python -m backend.cli.data_coverage` 或直接查库,不要相信
> 本节手工矩阵的旧快照。
