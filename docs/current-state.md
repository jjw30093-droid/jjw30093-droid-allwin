...
## 57. 角球「数据倾向」恒为空——整数盘口线等价映射 + 空白态改数字(2026-08-21)

### 背景与生产实测

站长反馈角球卡倾向永远是空的。抽样生产 API(`/api/v1/matches?status=
upcoming&window=7d&limit=60`)测得:全部未开赛比赛能给出角球倾向的
仅 **8/3876 = 0.21%**;同一区块的罚牌卡因为默认线 3.5 已定级,同期
**52/60 有倾向**——同一套代码、同一份 100% 覆盖的原始角球统计
(`fact_team_match_stats.extra_json.corners`),差距完全在标定/盘口线
匹配层,不是缺数据。

根因是两件事叠加(§42.5/§56 已分别记录,本次是首次定量拆开、给出修复):

1. **真实盘口线是整数,回测只标定了半线。** 全库 `fact_team_match_stats`
   28,506 行角球值逐行核实 **100% 为整数,0 例外**;而 Bet365 真实角球线
   `8.0/9.0/10.0/11.0` 占样本 **34/71 = 47.9%**,`market_calibration` 只标定
   了 `[8.5, 9.5, 10.5]`,`_bucket_lookup` 按 `line` 精确相等查表,整数线
   一律查不到 → `no_calibration/line_not_calibrated`。
2. **默认线 9.5 恰好是唯一未通过稳定性门槛的角球线**(§42.5 已记录:
   `spread_pp=7.9`,`monotonic=0`,五档命中率 0.470→0.461→0.515→0.494→
   0.539 有 2 处反向)——诚实的建模结论(过 9.5 的基准率 49.57%,接近
   抛硬币),没有真实盘口时默认线就是 9.5,角球没有真实盘口的
   3806/3876 场因此天然落在这条空线上。

### 修复:整数线等价映射(Fix A)

角球数是整数,对整数随机变量 `X` 与整数线 `L`:`X>L ⟺ X>L+0.5`——整数线
`L` 与半线 `L+0.5` 的"过线"事件完全等价,`L+0.5` 档的样本外命中率可以
原样复用于 `L`。这是精确等价,不是估算,不需要重跑 `calibrate_markets.py`,
不修改 `market_calibration` 表数据。

`backend/queries/market_cards.py` 新增 `_calibration_line(market, line)`:
`line` 已在 `market.lines` 内原样返回;否则 `line` 为整数且 `line+0.5` 已
标定则返回 `line+0.5`;其余(如四分之一线)返回 `None`,继续走
`no_calibration`——不给非整数线瞎猜映射。`_bucket_lookup`/`_line_calibration`
改用映射后的线查表;`lines[]` 折叠区的 `is_default` 同步改为与**生效线**
比较,不再与原始 `market.default_line` 比较(修掉一个既有小缺陷:真实线
10.0 时此前 `lines[]` 里没有任何一行被标默认)。

`MarketCardDTO` 新增 `calibration_line: Optional[float]` 字段,记录实际
用于查表的线;等于 `line` 时前端不提示,不等于时折叠区渲染等价说明并
披露真实盘口整数线的走水口径差异(本站 `hit_rate` 定义是 `P(总数>线)`,
不含真实盘口整数线的打平退款口径)。

**这一改动只影响读侧的映射与查表,不改 `calibrate_markets.py` 的
`MARKETS` 定义、不重跑标定、不写 `market_calibration`。**

### 修复:空白态从空话改成事实对比(Fix B)

Fix A 之后仍然空着的卡(9.5/9.0 等映射后仍未定级的线、场次不足)此前
渲染的是"暂无倾向"+ 一个孤零零的 `—`,两格全是空话。改为按
`card.estimate != null` 分两种:有 estimate 时两格都填真实数字——左格
两队近 10 场合计、右格带符号的差值(如 `+0.9`),**不出现任何方向词**;
无 estimate(`insufficient_sample`/`no_history`)时维持原状——真的什么都
没有,不硬凑。头部降级 pill 与下方解释段落全部保留,`hit_rate` 在降级态
依然不渲染。样式沿用既有 `.leanCellEmpty` 虚线弱化风格(新增
`.factCell`/`.factValue`/`.factCaption`),与已定级的实底青绿 `.leanCell`
明显区分;数字本身不做红绿判断色(降级态没有方向性结论,不能靠颜色
暗示方向)。

### 量化效果(生产只读实测,同一批 60 场未来 7 天比赛,同一时刻前后对拍)

按"旧逻辑只有 line 本身恰好等于已标定线(8.5/9.5/10.5)才可能有 grade"
反推旧逻辑基线:**旧逻辑 1/60 有倾向,新逻辑 6/60**——5 张卡从"未标定"
变成真实回测倾向(2 张 8.0→8.5 ★、3 张 10.0→10.5 ★;这批样本恰好只有
5 张 10.0 线,不同于会话早前一次审计看到的 16 张,NowGoal 角球盘覆盖率
本身很低且随时间窗口变化,量级会随抽样时刻自然波动,不代表实现有问题)。
其余 9.0 线映射到 9.5 后仍未定级(如实展示为"样本外测试不稳定"而不是
"未标定",从"没查过"变成"查过但不稳定",更诚实),11.0 线映射到 11.5
仍未标定(11.5 从未跑过回测)。

### 验证

- `tests/backend/test_market_cards.py`:改写了被本次修复直接推翻的
  `test_real_integer_line_reports_line_not_calibrated`(线 10.0 此前断言
  `no_calibration`,现在正确断言 `ok`+★★+真实 `lean`),新增映射到未定级
  线(9.0→9.5)、四分之一线不映射(9.25)两个用例,`market_fixture` 新增
  `_seed_calibration(..., graded=False)` 支持复刻"已标定但未定级"这个
  真实生产状态(此前测试库只有"从未标定"一种空场景)。全部 18 个用例
  通过。
- `frontend/tests/market-card.test.tsx`:改写了 estimate 非空时的降级态
  断言(从断言"暂无倾向"改为断言两个事实数字),新增 estimate 为
  null 时仍回退旧空态、`calibration_line` 与 `line` 不同/相同两种等价
  说明渲染的用例。全部 17 个用例通过。
- 浏览器实测(本地真实数据,深浅两套主题、375px 移动端宽度):真实线
  10.0 场次(千叶市原联赛)结论区出现"★ 偏小 41%",折叠区渲染等价说明
  与走水披露;真实线 9.5 场次结论区改为"10.4/两队近10场合计"+
  "+0.9/对比盘口线",无方向词;`insufficient_sample` 场次(千叶市原主队
  仅 1 场历史)维持原"暂无倾向/场次不足"。
- `CI=1 npx playwright test e2e/anonymous.spec.ts -g 数据倾向`:运行时发现
  该 spec 断言罚牌卡"偏大"失败(实际"偏小 37%")——用 `git stash` 验证
  这条失败在改动前的 main(`d758191`)上同样 100% 复现,是与本次角球改动
  无关的既有 E2E 数据漂移(seed 脚本对 yellow_cards 的标定依赖当时线上
  `data/allwin.db` 的真实历史,随时间增长会漂移),已作为独立任务
  (task_4bf659d5)转出、未在本次范围内处理;角球卡本身的渲染内容通过该
  失败测试的页面快照直接核对无误(等价映射说明、事实数字、"样本外测试
  中不够稳定"文案均正确出现,页面上没有任何"偏大"/"偏小"泄漏)。
- 每提交通用 `tsc --noEmit`/`eslint`/`vitest`(405 passed)/`npm run build`/
  `pytest tests/backend`/`npm run check:api-drift` 全部通过。

### 本轮不做(已记录)

- 把 11.5 线纳入离线标定(仅影响 1-2 场,需重跑标定跑批);
- 重新设计角球预测因子(现在只用两队自身角球均值之和,`touches_opp_box`/
  `accurate_crosses` 等驱动因子从未进入预测式);
- 9.5 线不单调的问题本身(不绕过门槛,这是正确行为);
- `market_calibration` 至今只有 `league_id=0` 跨联赛行,分联赛标定从未
  跑过(§42.8 已记录);
...
## 65. 质量门 G15 增加「已知缺口」登记 + xref 人工审核队列核对(2026-09-10)

### 65.1 G15_ACCEPTED_GAPS(站长拍板:两个联赛的历史赛程不回填)

生产 `fixture_round_gap` 从 2026-08-30 起连续 CRITICAL 401 次,查明是两个
(联赛, 赛季) 的结构性缺口:

| 联赛 | 赛季 | 实存轮次 | 缺 |
|---|---|---|---|
| 英冠(48) | 2025/2026 | 仅 37–46 | 36 轮 / 432 场 |
| 巴甲(268) | 2026 | {4, 21, 23–38} | 20 轮 / 215 场 |

成因是 CLAUDE.md §6.3 的已知结构性问题(赛程同步只写未完赛比赛,中途接入
的联赛在接入前踢完的场次永久缺失)。**巴甲那 215 场正是 §6.3 点名的那次
事故本身**——当时写明"必须补跑一次 `backfill_fixtures`",一直没跑。

2026-09-10 站长决定:**不回填**(网站只看当前采集)。据此给 G15 加
`G15_ACCEPTED_GAPS`,与既有的 `G15_EXEMPT` 是**两个不同概念**,不要混用:

- `G15_EXEMPT`:赛制本来就这样,不算缺口 → 整条跳过;
- `G15_ACCEPTED_GAPS`:确实缺了、我们知道、决定不修 → **照常检测、照常在
  返回值的 `accepted_gaps` 里逐条报(文本输出也打印),只是不抬高门的级别**。

两点设计取舍写在常量注释里:
1. 不做成 `continue` 静默跳过——静默消失比永远红更危险,缺口还在只是没人
   看得见;也不保留永远红——本文件 G10 与 G15 注释里两次记过"两周内被当成
   噪音关掉"的教训。
2. value 是**接受的缺口规模基线**(`missing_count`),不是布尔开关:实际缺口
   超过基线 → 说明出了新问题,照常 CRITICAL 并标 `exceeds_accepted_baseline`。
   与 G12 `season_label_drift` 的"存量基线 + 只报新增漂移"同一套路。

回归防线没有削弱:`test_268_shape_critical` 那条事故回归测试原样保留(断言
搬到 `accepted_gaps`,检测逻辑一旦退化条目会整个消失、断言照炸),另新增
两条——同联赛**其它赛季**仍 CRITICAL、缺口**超出基线**仍 CRITICAL。

### 65.2 xref 人工审核队列核对(65 条)

`dim_match_xref.review_status='needs_review'` 积压 65 条。核对结论:

- **只有 7 条是活的**。G11 只看未来 168h;其余 57 条已完赛 + 1 条已取消,
  采集窗口早过,确认与否都不会多采到一条赔率。
- confidence 的含义(`entity_resolution.py`,`_SIDE_SCORE_EXACT=0.5` /
  `_SIDE_SCORE_FUZZY=0.35` / `AUTO_OK_THRESHOLD=0.9`):1.0=两边精确、
  0.85=一边精确+一边模糊、0.7=两边模糊、**0.5=一边精确+另一边零信号**
  (队列里的绝大多数)。
- **抓到一条真错配**:热刺 vs 埃弗顿(fm 5795454 / ng 2921246)——拉 NowGoal
  当日日程实测,2921246 是**智利甲的 `Ñublense vs Everton CD`**,不是英超
  埃弗顿。这是 §6.1 记过的同名噪声陷阱,而且它占着
  `UNIQUE(provider, fotmob_match_id)` 名额**挡住真候选**(代码注释明写
  "occupied 检查不分 review_status"),导致这场英超采不到赔率。
- **已确认 4 条**(2026-09-10,写库前跑了 `backup_sqlite.sh`,备份
  `20260910T133018Z`):
  - PSV vs 斯巴达鹿特丹(5781743,conf 1.0):NowGoal 侧队名实测就是
    `'PSV Eindhoven' vs 'Sparta Rotterdam'`,两边精确,当日荷甲唯一一场;
    `kickoff_diff=-19800` 是 NowGoal 的开球时刻不准(FM 侧 `exact` +
    `fotmob:fixtures`,同日 12:30 档位被海伦芬 vs 特尔斯达占着);
  - 曼联 vs 萨巴赫(6106293,conf 0.85)与勒沃库森 vs 采列(6112244,conf 0.85):
    kickoff 精确一致 + Bet365 1x2 形态吻合(1.08/11.0/26.0、1.17/7.0/15.0)。
  - 布拉格斯拉维亚 vs 朗斯(6106392,conf **0.5**):**站长当日单独拍板确认**。
    证据只有 kickoff 精确一致 + Bet365 1x2 形态吻合(2.60/3.60/2.50,两队实力
    接近),**没有队名实证**——那个 titan_id 已从当日日程接口消失(现在只返回
    U19 场次 `Slavia Praha U19 vs Lens U19`,顺带验证了 `_squad_qualifiers`
    的青年队否决逻辑有效),赔率端点又只给赔率不给队名。确认的直接动机是它
    4.9 小时后开球,不确认这场赔率就永久采不到。**这条的证据等级低于上面
    三条,如实记录,不要拿它当"0.5 也可以确认"的先例。**
- **未确认**(conf 0.5,同样只有旁证):奥林匹亚科斯 vs 亚盖沃尼亚(6112268)
  与巴黎 vs 雷恩(5802907,2027-03,NowGoal 既无日程也无赔率,连旁证都没有)。
  按 §6.1「不得静默写成已验证」留在队列里。
  按 §6.1「不得静默写成已验证」保留在队列里。

确认后 G11 `xref_unmapped_upcoming` 从 **CRITICAL 降到 WARNING**(近 48h 不可采
清零,far_unpollable=3)。
