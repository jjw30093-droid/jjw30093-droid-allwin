/**
 * matchTeamColors.ts 单测(2026-08-24)。
 *
 ...
 * 所以必须直接单测这个函数本身的行为:主题变体选对、对比度不达标时正确
 * 回退、缺失/非法输入不崩溃、不跨主题借用另一份颜色。
 */
...
describe("resolveTeamColor", () => {
  ...
  it("深色模式变体本身对比度不达标(真实案例:#035db8 对深色球场 #333333 只有 1.96:1)时回退品牌色", () => {
    const v = resolveTeamColor(REAL_AWAY_PAIR, {
      isDark: true,
