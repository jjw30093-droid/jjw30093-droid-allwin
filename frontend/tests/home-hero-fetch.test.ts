 ...
 *
 * 2026-08-19 候选池 `limit=8` → `limit=70`;2026-09-26 再 → `limit=120`(下面两处断言同步改)。
 ...
 * 本需求在最该生效的周末失灵。
 * 2026-09-26 新增"五大联赛 + 欧冠 48h 内优先"规则,候选窗口变成滚动 48h,实测最密
 * 115 场(3518 场未来赛程),70 不够,改 120。
 *
...
describe("fetchHomeData(HomeMatchExperienceLive.tsx,浏览器挂载后刷新)", () => {
  ...
  it("重点位候选请求(window=7d&limit=120)必须带 boost=free_predicted", async () => {
    const calls: string[] = [];
    ...
    const heroCall = calls.find((u) => u.includes("window=7d") && u.includes("limit=120"));
